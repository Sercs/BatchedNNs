...
class BatchLinearMasked(nn.Module):
    ...
    def _init_masks(self):
        self.weight_mask = torch.zeros((self.n_linears, self.n_out, self.n_in))
        self.bias_mask = torch.zeros((self.n_linears, self.n_out))
        with torch.no_grad():
    ...
    def forward(self, x):
        if self.add_residual:
            for _ in range(self.n_recurs+1):
                x = x + self.activation(torch.einsum('bni,nji->bnj', x, self.weights * self.mask) 
